...
export class Parser {
  ...
  public parseObjectWithSelections(
    ...
    for (const selection of selections) {
      data = this.treatSelection(data, fieldMap, selection);
    }
    return data;
  }
  ...
  protected treatSelection(
    data: DataRecord,
    fieldMap: GraphQLInputFieldMap | GraphQLFieldMap<unknown, unknown>,
  ...
  protected parseScalar(value: unknown, type: GraphQLScalarType): unknown {
    const fns = this.functionsMap[type.name] ?? type;
    return fns.parseValue(value) as unknown;
  }

  protected validateEnum(value: unknown, type: GraphQLEnumType): void {
    if (!this.validateEnums || !value) return;

    const enumValues = type.getValues().map((v) => v.value as unknown);
  ...
  protected parseNestedObject(
    ...
    if (!isRecord(value) || !fieldNode.selectionSet?.selections.length) {
      return value;
    }
  ...
  protected getObjectTypeFrom(
    ...
    const valueType = this.schema.getType(typename);
