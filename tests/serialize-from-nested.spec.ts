...
import { cloneDeep, isNumber, isString, observableOf, nextValue, dummyClient } from "./test-utils";
  ...
  const typesMap = {
    StartOfDay: {
      ...
      parseValue: (raw: unknown): CustomDate | null => {
        ...
        throw new Error(
          `given date to parse is not a string or a number!! (typesMap.StartOfDay.serialize): ${raw}`
        );
      }
    }
  };
...
`;
  ...
  it("use the scalar resolvers in the schema to serialize (without removeTypenameFromInputs)", async () => {
    ...
    const observable = execute(link, cloneDeep(request), { client: dummyClient });
